...
export default class TouchTexture {
	...
	initTexture() {
		...
		this.canvas.id = 'touchTexture'; // initializing the texture
	...
	addTouch(point) {
		...
		let vx = 0;
		let vy = 0;
		const last = this.last; // this.last was null in the beginning
		if (last) {
			const dx = point.x - last.x;
			const dy = point.y - last.y;
			if (dx === 0 && dy === 0) return; // reutrning nothing bc there is no distance between last point adn current point
			const dd = dx * dx + dy * dy; // area for the distance (x^2 + y^2)
			let d = Math.sqrt(dd); // distance between the two points
			vx = dx / d; // translating the anchor point to be the center
			...
			force = Math.min(dd * 10000, 1); // dd is a very small number so we make it bigger
